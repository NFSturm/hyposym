...
addprocs(2, topology=:master_worker, exeflags="--project=$(Base.active_project())")
...
drug_test = Alexandria.get_dataset("drug_interaction_test");

drug_test_data = JSONL.read_jsonl(drug_test["path"]);

mkdir("./context_paragraphs_test")
...
@everywhere begin
    ...
    function parse_drug_report(drug_report)
        parsed_doc = Stanza.nlp(drug_report["paragraph"])
        ...
        serialize("./context_paragraphs_test/$(doc_id).jls", DrugReport(doc_id, mentioned_drugs, extractions, parsed_doc))
    
    end
end

pmap(drug_report -> parse_drug_report(drug_report), drug_test_data)